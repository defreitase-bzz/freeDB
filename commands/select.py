...
def run(args):
    ...
    if join:
        ...
        ldx = idx_map[left]
        rdx = idx_map[right]
        cond_post = build_awk_cond(post_filters, idx_map)
        script = (
        ...
        cond_pre = build_awk_cond(pre_filters, idx_map)
        proj = build_awk_proj(columns, idx_map)
...
def build_awk_cond(filters, idx_map):
    parts = []
...
def build_awk_proj(columns, idx_map):
    if columns == ["*"]:
        return "$0"
    return ", ".join(f"$${idx_map[c]}" for c in columns)
